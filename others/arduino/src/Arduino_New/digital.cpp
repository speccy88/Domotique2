...
int digclass::ANALOG(int pin)
{
  ...
  if(14 <= pin <= 21)
    ...
    return(9998);                                            //Reply "9998" means the pin number is not valid for analog input read
}
...
int digclass::FREQ(int pin)
{
  ...
  {
    pinMode(pin,INPUT);
    Htime = pulseIn(8,HIGH);                                 //Read high time in microseconds (timeout default = 1s)
    Ltime = pulseIn(8,LOW);                                  //Read low time in microseconds (timeout default = 1s)

    TotalTime = Htime+Ltime;

    if(TotalTime > 0)
    {
      frequency = (1000000/(Htime+Ltime))*10;                     //Calculate frequency
      return(frequency);
    }
    else
